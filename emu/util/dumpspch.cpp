...
// Description:
//
// Copyright (c) 2002-2003 Marc Rousseau, All Rights Reserved.
//
...
static const UCHAR *vsmDataPtr;
static int          vsmBytesLeft;
static int          vsmBitsLeft;
static int          vsmData;

sNode *ReadNode ( const UCHAR *rom, int offset )
{
    ...
    const UCHAR *ptr = &rom [ offset + length + 1 ];

    int prevOffset = ( ptr [0] << 8 ) | ptr [1];
    int nextOffset = ( ptr [2] << 8 ) | ptr [3];
    int dataOffset = ( ptr [5] << 8 ) | ptr [6];
...
void DumpPhrase ( sNode *node, FILE *spchFile )
{
    ...
    if ( dataFormat == 0 ) {
        ...
        ERROR ( "Unrecognized data format (" << dataFormat << ")" );
    }

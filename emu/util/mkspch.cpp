...
struct sNode {
    ...
    {
        FUNCTION_ENTRY ( NULL, "strndup", false );
...
void WriteBits ( int data, int count )
{
    ...
    if ( data >= ( 1 << count )) {
        WARNING ( "data (" << data << ") contains more than " << count << " bits" );
    }
...
void ReadParsedData ( const char *ptr, sNode *node )
{
    ...
    try {
        ...
        if ( vsmBitsLeft != 8 ) {
            *vsmDataPtr++ = ( UCHAR ) ( vsmData << vsmBitsLeft );
        }
...
sNode *ReadNode ( FILE *file )
{
    ...
    do {
        line++;
        if ( fgets ( buffer, sizeof ( buffer ), file ) == 0 ) return NULL;
        ptr = buffer;
        while ( isspace ( *ptr )) ptr++;
    } while (( *ptr == '#' ) || ( *ptr == '\0' ));
...
int CalculateOffsets ( sNode *root, sNode **list, int count )
{
    ...
    for ( int i = 0; i < count; i++ ) {
        ...
        for ( int j = 0; j < i; j++ ) {
            if ( list [j]->d_length >= node->d_length ) {
                ...
                int offset = list [j]->d_length - node->d_length;
                if ( memcmp (( char * ) list [j]->data + offset, node->data, node->d_length ) == 0 ) {
                    node->copy = true;
...
UCHAR *StoreNode ( const sNode *node, UCHAR *ptr )
{
    FUNCTION_ENTRY ( NULL, "StoreNode", true );

    ptr += sprintf (( char * ) ptr, "%c%s", node->p_length, node->phrase );

    int offset;

    offset = node->prev ? node->prev->phrase_offset : 0;
    *ptr++ = ( UCHAR ) ( offset >> 8 );
    *ptr++ = ( UCHAR ) ( offset & 0xFF );
    ...
    *ptr++ = ( UCHAR ) ( '\0' );              // Dunno what this is yet
    *ptr++ = ( UCHAR ) ( offset >> 8 );
    *ptr++ = ( UCHAR ) ( offset & 0xFF );
    *ptr++ = ( UCHAR ) ( node->d_length );

    return ptr;
}

void MakeROM ( const sNode *root, sNode **list, int count, UCHAR *rom )
{
...
if ( vsmBytesLeft > 0 ) fprintf ( stdout, "Phrase: \"%*.*s\" has %d extra bytes\n", node->p_length, node->p_length, node->phrase, vsmBytesLeft );

    return vsmBytesLeft;
}

int CheckData ( sNode *node, int *phrases, int *unique, int *data_used, const UCHAR *rom, UCHAR *flags )
{
...
void PrintStats ( sNode *root, const UCHAR *rom )
{
    UCHAR flags [ 0x8000 ];
...
int main ( int argc, char *argv[] )
{
    ...
    int size = CalculateOffsets ( root, list, count );

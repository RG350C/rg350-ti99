...
#include "../include/tms9919-sdl.hpp"
...
bool ReadNode ( cTMS5220 *speech, ULONG address, sNode *node )
{
    FUNCTION_ENTRY ( NULL, "ReadNode", true );

    memset ( node, 0, sizeof ( node ));

    speech->WriteData (( UCHAR ) ( 0x40 | (( address >>  0 ) & 0x000F )));  // Load Address
    speech->WriteData (( UCHAR ) ( 0x40 | (( address >>  4 ) & 0x000F )));  // Load Address
    speech->WriteData (( UCHAR ) ( 0x40 | (( address >>  8 ) & 0x000F )));  // Load Address
    speech->WriteData (( UCHAR ) ( 0x40 | (( address >> 12 ) & 0x000F )));  // Load Address
    speech->WriteData (( UCHAR ) ( 0x40 | (( address >> 16 ) & 0x000F )));  // Load Address
...
ULONG LocateString ( cTMS5220 *speech, const char *string, int length )
{
    ...
    for ( EVER ) {
        ...
        if ((( int ) strlen ( node.string ) == length ) && ( delta == 0 )) break;
...
void SayPhrase ( cTMS5220 *speech, const char *text, int length )
{
    FUNCTION_ENTRY ( NULL, "SayPhrase", true );

    ULONG address = LocateString ( speech, text, length );
    if ( verbose > 1 ) {
        fprintf ( stdout, "Phrase: %*.*s Address: %04X\n", length, length, text, address );
    }
    ...
    speech->WriteData (( UCHAR ) ( 0x40 | (( address >>  0 ) & 0x000F )));
    speech->WriteData (( UCHAR ) ( 0x40 | (( address >>  4 ) & 0x000F )));
    speech->WriteData (( UCHAR ) ( 0x40 | (( address >>  8 ) & 0x000F )));
    speech->WriteData (( UCHAR ) ( 0x40 | (( address >> 12 ) & 0x000F )));
    speech->WriteData (( UCHAR ) ( 0x40 | (( address >> 16 ) & 0x000F )));

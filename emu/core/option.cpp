...
void PrintHelp ( int count, sOption *optList )
{
...
static sOption *FindOption ( const char *arg, int count, sOption *optList )
{
    if ( *arg != '-' ) {
        ...
        for ( int i = 0; i < count; i++ ) {
            ...
            int max = ( ptr == NULL ) ? strlen ( longname ) : ptr - longname;
...
int ParseArgs ( int index, int argc, char *argv [], int count, sOption *optList )
{

...
psp_menu_display_save_name()
{
  ...
  snprintf(buffer, 30, "Game: %s", TI99.ti99_save_name);
...
# if defined(LINUX_MODE) || defined(WIZ_MODE)
      struct tm *my_date = localtime(& TI99.ti99_save_state[slot_id].date);
...
# endif
  ...
  if (TI99.ti99_save_state[cur_slot].thumb) {
    psp_sdl_blit_thumb(170,125, TI99.ti99_save_state[cur_slot].surface);
  } else {
...
psp_main_menu_reset(void)
{
  ...
  psp_sdl_back2_print(  120, 110, "Reset TI99 !", PSP_MENU_WARNING_COLOR);
  psp_sdl_flip();
  ti99_reset_computer();
  sleep(1);
}
...
psp_main_menu_load(int format)
{
  ...
  {
    psp_display_screen_menu();
    psp_sdl_back2_print(  120, 110, "File loaded !", 
                       PSP_MENU_NOTE_COLOR);
    psp_sdl_flip();
    sleep(1);
    return 1;
  ...
  {
    psp_display_screen_menu();
    psp_sdl_back2_print(  120, 110, "Can't load file !", 
...
psp_main_menu_load_slot()
{
  ...
  if (! TI99.ti99_save_state[cur_slot].used) {
    ...
    psp_display_screen_menu();
    psp_sdl_back2_print(  120, 110, "Slot is empty !", 
                       PSP_MENU_NOTE_COLOR);
    psp_sdl_flip();
    sleep(1);
...
psp_main_menu_save_slot()
{
  ...
  {
    psp_display_screen_menu();
    psp_sdl_back2_print(  120, 110, "File saved !", 
                       PSP_MENU_NOTE_COLOR);
    psp_sdl_flip();
    sleep(1);
  }
  ...
  {
    psp_display_screen_menu();
    psp_sdl_back2_print(  120, 110, "Can't save file !", 
                       PSP_MENU_WARNING_COLOR);
    psp_sdl_flip();
    sleep(1);
  }
...
psp_main_menu_del_slot()
{
  ...
  {
    psp_display_screen_menu();
    psp_sdl_back2_print(  120, 110, "File deleted !", 
                       PSP_MENU_NOTE_COLOR);
    psp_sdl_flip();
    sleep(1);
  }
  ...
  {
    psp_display_screen_menu();
    psp_sdl_back2_print(  120, 110, "Can't delete file !", 
                       PSP_MENU_WARNING_COLOR);
    psp_sdl_flip();
    sleep(1);
  }
...
psp_main_menu_exit(void)
{
  ...
  psp_display_screen_menu();
  psp_sdl_back2_print(  120, 110, "press B to confirm !", PSP_MENU_WARNING_COLOR);
  psp_sdl_flip();
...
psp_main_menu_editor()
{
  ...
  sprintf(TmpFileName, "%s/txt/%s.txt", TI99.ti99_home_dir, TI99.ti99_save_name);
  ...
  while (! end_menu)
  {
        ...
        (new_pad == GP2X_CTRL_CIRCLE))
    {
      ...
      switch (cur_menu_id ) 
      {
        ...
        case MENU_LOAD_CART : if (psp_main_menu_load(FMGR_FORMAT_CTG)) end_menu = 1;
                              old_pad = new_pad = 0;
        break;              
